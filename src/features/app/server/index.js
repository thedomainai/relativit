...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const recentVerification = db.prepare(
      'SELECT * FROM verification_codes WHERE email = ? AND used = 1 AND expires_at > datetime("now", "-10 minutes")'
    ).get(email);

    if (!recentVerification) {
      return res.status(400).json({ error: 'Email not verified. Please start over.' });
    }
